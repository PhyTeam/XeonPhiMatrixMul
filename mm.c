...
void generate_large_square_matrix(int size)
{
    int num_of_block_per_row = size / MIN_SQUARE_MATRIX;
    int num_of_block_per_col = size / MIN_SQUARE_MATRIX;
    int num_of_elements_per_block = MIN_SQUARE_MATRIX * MIN_SQUARE_MATRIX;
    // for each block
    srand(time(0));
    int i, j, k;
    for(i = 0; i < num_of_block_per_row; i++){
        for(j = 0; j < num_of_block_per_col; j++){
            // generate block
            for(k = 0; k < num_of_elements_per_block; k++){
                rand() % 10;
            }
        }
    }
