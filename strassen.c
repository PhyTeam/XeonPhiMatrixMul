...
float* strassen_matrix_multiply_recursive(const REAL_PTR A, const REAL_PTR B, REAL_PTR out, int size){
    // allocate memory contain result
    REAL_PTR ptr_C = ALLOC_MATRIX(size, float);

    if (size <= MIN_SQUARE_MATRIX) {
#ifndef USING_ZMORTON
        int i, j, k;
        float sum;
...
#else
        simple_mm_using_zmorton(A, B, out, size);
#endif
        //return;
    } else {
        int i, j;
...
/*
    for(i = 0; i < size; ++i){
        for(j = 0; j < size; ++j){
            uint64_t idx = mortonEncode_for(i, j);
            printf("%.2f ", matrix_A[idx]);
        }
        printf("\n");
    }
    float* pMtrixA12 = matrix_A + (1 * size * size / 4);
    // partition matrix A
    for(i = 0; i < size / 2; i++){
        for(j = 0; j < size / 2; j++){
            uint64_t idx = mortonEncode_for(i, j);
            printf("%.2f ", pMtrixA12[idx]);
        }
        printf("\n");
    }
*/
...
#pragma omp parallel
#pragma omp single nowait
    strassen_matrix_multiply_recursive(matrix_A, matrix_B, matrix_C, size);
    stop = omp_get_wtime();
    elapsed = (double)(stop - start);
    strassen_time = elapsed;
    printf("Time elapsed in ms: %f\n", elapsed);
    write_file(matrix_C, size, "strassen.bin");
